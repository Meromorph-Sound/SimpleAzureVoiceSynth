...
public:
    ...
    bool ApplyModels(std::vector<std::shared_ptr<LanguageUnderstandingModel>> collection)
    {
        bool result = true;
        AZACHR hresult;
        PatternMatchingModel * simpleModel;
        ...
        for (auto model : collection)
        {
            ...
            case LanguageUnderstandingModel::LanguageUnderstandingModelType::PatternMatchingModel:
                simpleModel = static_cast<PatternMatchingModel *>(model.get());
                for (auto simpleIntent : simpleModel->Intents)
                {
                    hresult = intent_recognizer_add_simple_language_intent(m_hreco, simpleIntent.Id.c_str(), &simpleIntent.Phrases[0], simpleIntent.Phrases.size());
                    if (SPX_FAILED(hresult))
                    {
                        SPX_TRACE_ERROR("Invalid Argument: All Intents must have at least one phrase.");
                        SPX_THROW_HR(hresult);
                    }
                }
                break;
            default:
